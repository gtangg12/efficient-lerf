        ...
        if isinstance(self.render_setting, float):
            ...
            with torch.no_grad():
                clip_output = self.lerf_field.get_output_from_hashgrid(
                    lerf_samples, 
                    hashgrid_field, 
                    torch.full(clip_scales.shape, scale, device=lerf_weights.device, dtype=hashgrid_field.dtype)
                )
            return {'clip': self.renderer_clip(embeds=clip_output, weights=lerf_weights.detach())}
